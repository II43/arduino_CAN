 ...
 *  ------------------------------------------------------------------------- 
 *
 * Created: Fri Nov 10 10:09:49 2017
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME arduino_CAN_rtr_msg
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS            1
/* Input Port  0 */
#define IN_PORT_0_NAME        isrequested
...
extern void arduino_CAN_rtr_msg_Outputs_wrapper(const uint8_T *isrequested,
...
{
    const uint8_T *isrequested = (uint8_T *) ssGetInputPortRealSignal(S, 0);
    const int_T   p_width0  = mxGetNumberOfElements(PARAM_DEF0(S));
    const uint16_T *CAN_Id = (const uint16_T *) mxGetData(PARAM_DEF0(S));
    
    arduino_CAN_rtr_msg_Outputs_wrapper(isrequested, CAN_Id, p_width0);
}
